...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!timetableCaptureArea) {
        console.error("Timetable capture area element (#timetableCaptureArea) not found on DOMContentLoaded!");
    }
...
function checkAndEnableDownloadButton(buttonId) {
    ...
    if (btn) {
        ...
        console.warn(`Download Timetable button with ID '${buttonId}' not found for enablement check.`);
    }
}
...
window.showDaySelectionModal = function() {
    ...
    const sortedDays = Array.from(availableDays).sort((a, b) => {
        ...
        return (dayOrder[a.toLowerCase()] || 99) - (dayOrder[b.toLowerCase()] || 99); // Use 99 for unknown days, usually errors.
    });
    ...
    container.className = "bg-gray-800 rounded-lg p-6 max-w-md mx-auto text-center border-2 border-cyan-500 shadow-lg";
    ...
    if (sortedDays.length === 0) {
        ...
        noDaysText.innerText = 'No favorite artists found with valid day information.';
        buttonGroup.appendChild(noDaysText);
    } else {
        sortedDays.forEach(day => {
            const dayButton = document.createElement("button");
            dayButton.className = "px-6 py-3 bg-cyan-600 text-white rounded hover:bg-cyan-500 transition text-lg font-semibold";
            dayButton.innerText = day.charAt(0).toUpperCase() + day.slice(1); // Capitalize first letter
            dayButton.onclick = () => {
    ...
    closeButton.className = "mt-6 px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-500 transition";
...
async function generateVerticalTimetablePNG(selectedDay = null) {
    ...
    const favoriteSetKeys = new Set(state.favoriteSets.map(fav => fav.setKey));
    const getSetKey = (artist, day, stage, start) => `${artist}|${day}|${stage}|${start}`;
    ...
    ['arena', 'districtX'].forEach(venueKey => {
        ...
        Object.keys(state.data[venueKey]).forEach(dayKey => {
            ...
            if (selectedDay && dayKey.toLowerCase() !== selectedDay.toLowerCase()) {
                return;
            }
            Object.keys(state.data[venueKey][dayKey]).forEach(stageKey => {
                if (!Array.isArray(state.data[venueKey][dayKey][stageKey])) return;
                state.data[venueKey][dayKey][stageKey].forEach(set => {
                    const setKey = getSetKey(set.artist, dayKey, stageKey, set.start);
                    if (favoriteSetKeys.has(setKey)) {
                        if (set && set.artist && stageKey && set.start && set.end) {
                            ...
                            console.warn(`Skipping incomplete favorite artist data for setKey: ${setKey} due to missing properties.`, set);
                        }
                    }
                });
            });
    ...
    if (favoriteArtistsArray.length === 0) {
        customAlert.alert(`No favorite artist data found for ${selectedDay ? selectedDay : 'the selected days'} to generate a timetable.`);
        return;
    }
    ...
    sortedArtists.forEach(artist => {
        if (artist.day !== currentDayInContent) {
            ...
            htmlContent += `<h2 class="day-heading">${artist.day.charAt(0).toUpperCase() + artist.day.slice(1)}</h2>`;
        ...
        const escapedName = (artist.name || 'Unknown Artist').replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;").replace(/\r?\n/g, ' ');
        const displayTime = `${artist.start || 'N/A'} - ${artist.end || 'N/A'}`;
        const escapedTime = displayTime.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;").replace(/\r?\n/g, ' ');
    ...
    const updatedBottomSpacer = timetableCaptureArea.querySelector('.bottom-spacer');
    ...
    try {
        ...
        customAlert.alert("Could not generate the timetable image. An error occurred during capture. Check the console for details.");
    } finally {
        // --- Clean Up ---
        timetableCaptureArea.innerHTML = '';
        timetableCaptureArea.style.width = '';
        timetableCaptureArea.style.height = '';
        timetableCaptureArea.style.position = '';
        timetableCaptureArea.style.left = '';
        timetableCaptureArea.style.top = '';
        timetableCaptureArea.style.transform = '';
